...
#ifndef AANDUSB_YUV420p_YV12_FSH_H
#define AANDUSB_YUV420p_YV12_FSH_H
...
// YUV420p_YV12の3プレーンのデータをGL_LUMINANCEの1つのテクスチャとしてまとめて渡して換算する場合(元の1ピクセル分のデータがテクセル1個に対応)
void main() {
	...
	float v1 = texture2D(sTexture, vec2(uv_tex.x,           uv_tex.y)).r;
	float v2 = texture2D(sTexture, vec2(uv_tex.x + uv_sz.x, uv_tex.y)).r;
	v = (v1 + v2) / 2.0;
	float u1 = texture2D(sTexture, vec2(uv_tex.x,           uv_tex.y + uv_sz.y)).r;
	float u2 = texture2D(sTexture, vec2(uv_tex.x + uv_sz.x, uv_tex.y + uv_sz.y)).r;
	u = (u1 + u2) / 2.0;
...
constexpr const char *yuv420p_yv12_as_lumi_gl3_fsh =
...
void main() {
	...
	float v2 = texture(sTexture, vec2(uv_tex.x + uv_sz.x, uv_tex.y)).r;
	v = (v1 + v2) / 2.0;
	float u1 = texture(sTexture, vec2(uv_tex.x,           uv_tex.y + uv_sz.y)).r;
	float u2 = texture(sTexture, vec2(uv_tex.x + uv_sz.x, uv_tex.y + uv_sz.y)).r;
	u = (u1 + u2) / 2.0;
	// 輝度がEPS未満なら黒を返す(プレビューを停止した時に画面が緑になるのを防ぐため)
	if (y > EPS) {
		vec3 yuv = vec3(clamp(y + uBrightness, 0.0, 1.0), u, v) - YUV_OFFSET;
...
#endif //AANDUSB_YUV420p_YV12_FSH_H
