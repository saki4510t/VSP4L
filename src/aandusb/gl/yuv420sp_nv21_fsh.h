...
#ifndef AANDUSB_YUV420sp_nv21_FSH_H
#define AANDUSB_YUV420sp_nv21_FSH_H

// YUV420sp(NV21)のセミプレーンのデータをGL_LUMINANCEの1つのテクスチャとして渡して
// RGBAとして描画するためのフラグメントシェーダー
#if __ANDROID__
constexpr const char *yuv420sp_nv21_as_lumi_gl2_fsh =
...
void main() {
	...
	float u = texture2D(sTexture2, vTextureCoord).a;
	float v = texture2D(sTexture2, vTextureCoord).r;
...
constexpr const char *yuv420sp_nv21_as_lumi_gl3_fsh =
...
void main() {
	...
	float u = texture(sTexture2, vTextureCoord).a;
	float v = texture(sTexture2, vTextureCoord).r;
	// 輝度がEPS未満なら黒を返す(プレビューを停止した時に画面が緑になるのを防ぐため)
	if (y > EPS) {
		vec3 yuv = vec3(clamp(y + uBrightness, 0.0, 1.0), u, v) - YUV_OFFSET;
		vec3 rgb = clamp((MAT_YUV2RGB * yuv), 0.0, 1.0);
...
#endif //AANDUSB_YUV420sp_nv21_FSH_H
